...
BRANCH_Rx=branch_r3
GIT_REPO_DIR=/home/daily/softbank/kernel_imx_r3
R3_KERNEL_WORKING_DIR=git-on7-r3
REPO_DIR=$PWD
BRANCH_R3=branch_r3
ENV_DIR=/home/daily/work
SHA1_CODE=SHA1_CODE
KERNEL_WORKING_DIR=$ENV_DIR/imx51-kernel/trunk/
SOURCE_DIR=$KERNEL_WORKING_DIR/$R3_KERNEL_WORKING_DIR
...
git fetch origin
git rebase origin
#git pull error, remove it and clone a new one
if [ $? -ne 0 ]; then
    cd $KERNEL_WORKING_DIR
    if [ -e $R3_KERNEL_WORKING_DIR ]; then
        echo "Deleting $R3_KERNEL_WORKING_DIR folder"
        rm $R3_KERNEL_WORKING_DIR -fr
    fi
    echo "Clone source code..."
    nice git clone $GIT_REPO_DIR $R3_KERNEL_WORKING_DIR

fi
echo "Fetching commit messages"
#fetch the first version to the latest version commit messages
git log --pretty=format:'%h : %s' --date-order init..master > $RELEASE_LOG_FILE
