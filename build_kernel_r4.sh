...
RX_KERNEL_WORKING_DIR=git-on7-r3
REPO_DIR=$PWD
ENV_DIR=/home/daily/work
SHA1_CODE=SHA1_CODE
KERNEL_WORKING_DIR=$ENV_DIR/imx51-kernel/trunk/
SOURCE_DIR=$KERNEL_WORKING_DIR/$RX_KERNEL_WORKING_DIR
...
if [ $? -ne 0 ]; then
    ...
    if [ -e $RX_KERNEL_WORKING_DIR ]; then
        echo "Deleting $RX_KERNEL_WORKING_DIR folder"
        rm $RX_KERNEL_WORKING_DIR -fr
    fi
    echo "Clone source code..."
    nice git clone $GIT_REPO_DIR $RX_KERNEL_WORKING_DIR
